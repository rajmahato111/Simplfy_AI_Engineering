...
>
> **You:** "Great. So: 4M documents from three sources, 20K users, permission-true answers with citations, p95 under 4 seconds, 15-minute freshness for wikis, single-turn. If roughly 10% of employees ask a few questions a day, that's low single-digit queries per second at peak — so retrieval *quality* is the hard problem here, not throughput. I'll design for that."
...
>
> Bottom half is the query path. A user's question gets a light rewrite — expanding acronyms, cleaning phrasing. Then the permission filter resolves the caller's groups and injects an ACL predicate *into* the retrieval query itself, so a chunk the user can't read never even becomes a candidate. Hybrid retrieval runs dense and keyword search in parallel and fuses them. A cross-encoder reranks the top ~50 down to the 5–8 chunks that actually matter. Context assembly formats those with source labels, the LLM generates a cited answer, and we stream it back.
>
