...
Tool use is the mechanism that lets a model act instead of only talk. The model is shown a schema describing each available tool, it emits a structured call naming one tool and its arguments, the system actually executes that call, and the result comes back as an observation the model reasons over next. The Model Context Protocol (MCP) — released by Anthropic in November 2024 and adopted widely since — standardizes the wiring underneath that mechanism: instead of every application writing a bespoke integration for every tool provider, an MCP client (the agent's runtime) talks to any MCP server (a separate process exposing tools, data resources, and prompt templates over a shared JSON-RPC-based protocol) the same way, regardless of which model or framework is on the other end. What the protocol does *not* solve is judgment: whether an agent picks the right tool and fills in the right arguments depends almost entirely on how well each tool is described. A vague description is the single most common reason a capable model calls the wrong tool, or the right tool with the wrong arguments.

## The analogy

Picture a hotel concierge desk, and the laminated card of services sitting on it.

A guest doesn't know how to book a car to the airport, arrange dry cleaning, or reserve a table — they don't have the phone numbers, the account codes, or the standing relationships with local vendors. So they describe what they want to the concierge, who consults a card listing every service the hotel offers: what it does, how to request it, and — critically — what it doesn't cover. "Car service: local trips under 5 miles, 7 a.m. to 11 p.m., does not go to the airport; use Airport Shuttle for that." The guest picks the entry that matches, states the request in the shape the card expects ("destination, pickup time"), the concierge places the actual call, and relays back exactly what came back — "car confirmed for 6:15, blue sedan" — which the guest then acts on.

For years, every hotel did this differently: one had a phone list at the front desk, another trained bellhops to relay requests by hand signals across the lobby, a third had guests simply wander around asking employees directly and hoping someone knew the right extension. A guest who'd stayed at one hotel had to relearn the whole system at the next. What changed the picture was a shared standard: one card format, adopted chain-wide, listing services the same way and relaying results the same way, so any concierge at any property — and any guest who'd used one before — could work with it immediately. That's the shift MCP represents for tool integration: not a smarter concierge, just an agreed shape for the card and the phone call, so the same tool can be plugged into any agent without a bespoke integration each time.

The card's wording is where things actually go wrong or right. A card entry that just says "Car service" tells the guest nothing about its range — request an airport run and the driver arrives, discovers it's outside the service area, and the trip is wasted. The entry that spells out the boundary and points to the right alternative prevents that failure before it happens. The concierge system was never the weak link; an underspecified card always was.
...
Tool use itself is a three-step cycle, regardless of protocol. **Schema presentation**: the model is shown, in its context, a description of each tool it could call — a name, a set of typed arguments, and a description of what it does. **Intent and extraction**: the model emits a structured call — a tool name plus arguments — instead of prose. **Execution and contextualization**: something outside the model actually runs the call, and the real result is fed back into the model's context as the next observation, not as the model's guess about what the result probably was.

MCP standardizes the layer that feeds step one and carries out step three. Follow the diagram: the agent's **MCP client** — code inside the agent's runtime — talks to one or more **MCP servers**, each a standalone process that owns a set of tools (and optionally data resources and prompt templates) over JSON-RPC, either over local `stdio` or over HTTP for a remote server. Communication happens in two phases. First, discovery: the client calls something like `list_tools` and gets back the schemas for what that server currently exposes — the agent doesn't need every tool from every server hardcoded into its prompt, only the ones actually relevant to the current sub-goal get attached, which keeps the prompt lean as the number of connected servers grows. Second, invocation: once the model emits a call, the client routes it to the correct server, that server executes the real logic (hitting a database, calling an external API, reading a file) and returns a result, which the client hands back to the model as an observation.

The architectural payoff is the separation itself. Because each MCP server is its own process, a CRM integration, a ticketing integration, and a wiki-search integration can be built, scaled, and secured independently of the model orchestrator and of each other — a compromised or malformed call against one server doesn't automatically reach the others. That maps directly onto the concierge analogy's biggest structural change: not a smarter guest, and not a smarter concierge, but a common card format and phone system that any hotel, and any tool provider, can plug into the same way.

## A concrete example

A support agent is wired to three MCP servers: a CRM server (6 tools), a ticketing server (4 tools), and a wiki-search server (2 tools) — 12 tools, roughly 150 tokens per schema once you include the description and argument types, for about 1,800 tokens of schema text in total.

**Without discovery-based filtering:** every request hardcodes all 12 schemas into the prompt regardless of relevance. At 50,000 requests a day, that's 1,800 tokens of dead weight on requests that only ever need two or three of those tools — 90 million tokens a day spent on schemas the model never calls.

**With MCP-style discovery:** the client attaches only the tools relevant to the current sub-goal — typically 2 to 3 of the 12 — cutting the schema overhead to roughly 300–450 tokens per request, an 80% reduction, with no change to which tools are actually available when needed.

Separately, the team tested how much the *wording* of one tool's description mattered. The `search_tickets` tool originally read: "Searches tickets." Run against 50 real queries that should have used the separate `search_archive` tool instead (tickets resolved more than 90 days ago), the agent misused `search_tickets` — calling it, getting an empty result, and then guessing an answer instead of trying the archive tool — on 12 of 50 queries, a 24% misuse rate. The team rewrote the description: "Searches active support tickets by customer email or ticket ID; returns at most 20 results; does not include tickets resolved more than 90 days ago — use search_archive for those." Re-run against the same 50 queries, misuse dropped to 2 of 50 — 4%. The protocol connecting the tool never changed. The sentence describing its boundary did.
...
The dimension that matters more than protocol choice, and that no protocol can fix for you: description quality. A terse description is fast to write and reliably ambiguous — it says what a tool does but not what it doesn't, not its limits, not when to reach for something else instead. A precise description costs more upfront (and needs updating as the tool's behavior changes) but is what actually determines whether the model routes correctly. MCP standardizes *how* a schema gets to the model; it says nothing about whether that schema is good enough to act on.
