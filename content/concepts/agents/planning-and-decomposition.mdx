...
| A rope team sent ahead to fix lines while the group rests | A parallelizable sub-task, dispatched independently |
| Not renegotiating the route every morning on a known mountain | Committing to the plan — the payoff of upfront planning |
...
| Approach | Best when | Coordination cost | Failure mode |
|---|---|---|---|
| Plan-and-execute (static) | Domain is well understood; steps are largely independent | Low — one planning pass, steps parallelize | Brittle: one wrong early assumption invalidates every step downstream |
| Dynamic replanning | Environment can invalidate assumptions mid-task | Higher — a planning pass per checkpoint | Thrashing: replanning on noise instead of genuine crossings |
| Decomposition depth | Goal too large for one step or sub-agent to hold in context | Grows with depth; each level adds overhead | Runaway recursion — decomposing a task already small enough to execute |
