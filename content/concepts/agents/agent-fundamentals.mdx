...
The first is cruise control: set it to 65 and it holds that speed, reacting to one signal — current speed versus target. It has no idea there's an airport, a route, or a merge coming in half a mile. It's the simplest kind of automation, a feedback loop with zero judgment, and it is not an agent, however clever the algorithm underneath.

The second is a turn-by-turn navigation app. You give it a destination once, and it computes a full route in advance. It recalculates if you miss an exit, but the *shape* of what it can do was decided before you started driving. It never questions whether you actually want the airport, and it won't stop for gas because it noticed the tank is low — someone would have had to build that branch in ahead of time. It's a workflow: a fixed plan with a little built-in recalculation, not an open-ended decision-maker.

The third is calling a chauffeur: "Get me to the airport by 6, I have a 40-pound bag, and I'd rather pay a toll than be late." The chauffeur watches the road — actual traffic, an accident two exits up, the fact you're now running five minutes behind — and continuously decides what to do: take the toll road, skip the coffee stop, call ahead to the terminal. Nobody wrote the full route in advance, because nobody could have. That watch-decide-act cycle, repeating for as long as the trip takes, is the agent loop. Notice what you gave up to get it: you can't predict exactly which streets you'll take, and if the chauffeur makes a bad call, it happens on the road, not in a spec review.
...
Follow the loop in the diagram. Everything starts at **environment**: the current state the agent has to work with — a request, the result of the last tool call, a file, an alert. That state flows into **observe**, where the agent reads it. From there it flows into **think**: the model reasons over what it just observed and the goal it was given, and decides on a single next action. That decision becomes **act**: the agent calls a tool — a search, a database query, an API write — and something actually changes or gets revealed in the world. The result flows back into the environment, and the loop turns again.

The loop doesn't run forever. Two conditions end it, shown as the exit arrow leaving "think": the model decides the goal is satisfied and produces a final answer, or an external budget — a turn limit, a cost cap — forces a stop regardless of what the model thinks. That budget isn't optional: without it, a confused agent has no built-in reason to ever stop calling tools.
...
**Fixed 3-step workflow:** look up the account, apply the standard refund percentage for the plan tier, issue the refund. Three API calls, under a second, fully deterministic. It resolved 61% of tickets correctly. The other 39% were tickets where the customer had already received a partial refund in the last 30 days, was on a grandfathered plan, or wanted something adjacent to a refund. The workflow had no branch for "check refund history first," so it silently applied the wrong amount instead of failing loudly.

**Agent given the goal "resolve this refund request correctly per policy":** it observes the ticket, decides it needs the refund history first, calls that tool, observes a prior refund issued 12 days earlier, reasons a proration rule applies, checks the plan-tier table, computes the adjusted amount, issues the refund, and drafts a one-line explanation. That took 6 tool calls and about 21 seconds on average, at roughly 6 cents of model cost per ticket. Correctness: 89%. The remaining 11% were genuinely ambiguous cases the team routed to a human instead of letting the agent guess.

The agent didn't get better tools than the workflow had. It got permission to decide, per ticket, which tools to check and in what order.
...
4. **Assuming more autonomy is strictly an upgrade.** The freedom that solves a case nobody anticipated is the same freedom that makes behavior harder to guarantee under audit.
5. **Never benchmarking the agent against the workflow it replaced.** Teams ship the version that impressed the demo and skip the comparison against the boring deterministic baseline.

## The interview lens

Interviewers use this topic to see whether you reach for agents because a problem needs one, or because they're the exciting answer. The tell: can you name the specific uncertainty that justifies the loop?

A strong sound bite: *"I don't ask whether a system uses an LLM to decide if it's an agent — I ask whether the next action is chosen by the loop itself, from what it just observed, or whether a developer already knew every branch in advance. If a human could have written the full decision tree ahead of time, I'd rather ship the cheap, auditable workflow."*
