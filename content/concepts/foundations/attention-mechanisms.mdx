...
Take the sentence "The trophy didn't fit in the suitcase because it was too big," and focus on how "it" gets its updated representation. Say the model uses a key dimension of 64 per head. "It"'s query gets scored against every other token's key — including "trophy" and "suitcase," the two candidates "it" could plausibly refer to. Suppose the raw dot products come out as 9.2 for "trophy," 3.1 for "suitcase," and mostly small values elsewhere; dividing by the square root of 64 (which is 8) brings those down to about 1.15 and 0.39. Softmax over the full set of scaled scores turns that gap into a decisive weighting — roughly 71% of the output blend pulled from "trophy"'s value vector, about 9% from "suitcase," and 20% spread thinly across the rest. "It"'s new representation is that weighted blend, exactly the disambiguation the sentence requires.
