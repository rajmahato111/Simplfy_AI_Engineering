...
Two competing methods decide *which* weights get quantized how aggressively. **GPTQ** quantizes layer by layer, choosing each layer's quantized values to minimize squared reconstruction error against the original weights — an error-driven approach that doesn't look at how the model actually uses those weights. **AWQ** (Activation-aware Weight Quantization), the diagram's protected branch off the 4-bit tier, instead runs a small calibration pass, observes which weights see the largest activations in practice, and treats that small salient fraction — commonly cited around 1% — as too important to compress, keeping just those at higher precision while quantizing the rest normally. Because it optimizes for what the model actually uses rather than a generic reconstruction metric, AWQ tends to hold up better at aggressive bit-widths (3-bit) and on smaller models.
