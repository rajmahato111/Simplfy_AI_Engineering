...
1. **Stacking every pattern by default.** Decomposition, HyDE, multi-query, and self-query filtering running on every request multiplies latency and LLM cost for questions that needed none of it. Match the pattern to a diagnosed failure mode, same as [reranking](./reranking.mdx).
