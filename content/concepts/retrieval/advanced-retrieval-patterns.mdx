...
If your real question is really three questions bundled together — "is it the washer, the cartridge, or the supply line?" — the clerk splits it into three separate lookups instead of searching for "drips" as one blob. If you can't describe the part in the store's vocabulary — you know what it does but not its name — the clerk sketches what it probably looks like and searches the catalog by that sketch instead of your fumbling words. If you ask the same thing five different ways because you're not sure which phrasing will land, the clerk runs all five and keeps whatever shows up on multiple lists — corroboration, not repetition. If you say "under $20, in stock, for a 1998 model," the clerk narrows the whole catalog to that aisle before doing anything else — a filter, not a search. And if none of that fixes it, the clerk hands you a bag of "possibly relevant" parts and lets you sort by fit at the counter — fusion, deferring the final call to a later, more careful step.
...
**Query decomposition** targets compound questions — "compare our US and EU onboarding completion rates and explain the gap" is really three questions wearing one sentence. An LLM call splits it into sub-queries ("US onboarding completion rate," "EU onboarding completion rate," "reasons for the regional gap"), each retrieves independently, and the results are aggregated before generation.

**HyDE** targets the asymmetry between short queries and long documents. A three-to-ten-word query and a 300-to-500-word chunk live in different regions of embedding space, so their similarity score understates the true match. HyDE asks the LLM to write a one-paragraph hypothetical answer, then embeds *that* instead — its vocabulary and length resemble a real document, pulling it into the right neighborhood. The catch: if the query describes something that doesn't exist in the corpus, the LLM still writes a plausible hypothetical answer, and that hallucination becomes the search vector, confidently retrieving the wrong thing. The standard hedge is to also run the real query as a literal keyword search and fuse the two lists rather than trusting HyDE alone.
...
**Self-query with metadata filters** has the LLM parse the query into a semantic part and a structured part. "Enterprise contracts signed after 2024 mentioning termination clauses" becomes a filter (`year > 2024 AND doc_type = contract`) applied *before* the semantic search runs over "termination clauses." Cheap, and often the highest-leverage move of the whole list — it shrinks the candidate pool with zero embedding-space guesswork.

**Fusion** — usually reciprocal rank fusion (RRF) — lets you use several of the above at once without picking a winner up front. Score each document by summing `1 / (k + rank)` across every ranked list it appears in (k = 60 is standard), rewarding documents that rank well across multiple lists over one that tops a single list and appears nowhere else.
...
Baseline dense retrieval on the literal query scores the proration article at rank 41 of 50 — below the top-5 cutoff the UI shows. Two fixes, tested on a 200-query eval set:

- **HyDE**: the LLM writes a hypothetical paragraph — "This invoice discrepancy is likely caused by mid-cycle plan changes resulting in prorated charges..." — and that gets embedded. The article jumps to rank 3. Cost: one extra LLM call, ~200–400ms added latency.
- **Multi-query (4 paraphrases)**: "invoice discrepancy," "billing amount incorrect," "why did my charge change," plus the original. Fused with RRF, the article lands at rank 2. Cost: four retrieval calls instead of one, plus one LLM call for the paraphrases.

Both work. The team picked multi-query because their vector database handles concurrent queries cheaply but their LLM budget was the tighter constraint. Recall@5 on the eval set rose from 71% to 89%; latency grew from ~180ms to ~410ms per query — a tradeoff worth measuring, not assuming.
...
The throughline: every pattern here is a query-time tax, paid on every request, versus something like [contextual retrieval](./contextual-retrieval.mdx) or better chunking, which is an ingestion-time tax paid once. For high-QPS systems that difference dominates the decision — 300ms and an LLM call per query is a different bet at 10 queries/day than at 10,000 queries/minute.

## Where people go wrong

1. **Stacking every pattern by default.** Decomposition, HyDE, multi-query, and self-query filtering running on every request multiplies latency and LLM cost for questions that needed none of it. Match the pattern to a diagnosed failure mode, same as [reranking](./reranking-strategies.mdx).
2. **Trusting HyDE on out-of-corpus questions.** If the corpus doesn't contain the answer, HyDE still generates a fluent hypothetical — and confidently retrieves whatever is *nearest* to that hallucination. Pair it with a literal-query fallback.
3. **Treating self-query filters as free precision.** A misparsed filter doesn't degrade gracefully — it silently drops the correct document before semantic search even runs.
...
5. **Skipping fusion when combining signals.** Picking "whichever list ranks the result higher" instead of RRF throws away corroboration — a document ranking well across multiple query formulations is more trustworthy than one appearing in a single list.
...
- [Reranking](./reranking-strategies.mdx) — the complementary precision layer applied after these patterns produce a candidate pool.
