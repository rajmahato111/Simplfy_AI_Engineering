...
Looking up a fact is one trip to the shelf: you know the book, roughly the page, you grab it and you're done. That's plain RAG — one retrieval, one answer. Now think about researching something genuinely unfamiliar: "why did our biggest customer's usage drop last quarter?" You don't walk to one shelf. You pull the usage-metrics report first. Reading it, you realize the drop started after a support ticket spike, so *that* sends you to the support queue — not because you planned it up front, but because the first answer told you where to look next. There you find an unresolved billing complaint, which sends you to the billing system. Three trips, each chosen because the previous result demanded it — and if a shelf turns out empty, you go back and try a different one instead of reporting failure.

That back-and-forth, evidence-driven process — not a fixed itinerary decided in advance — is agentic RAG. The discipline that keeps it from becoming an afternoon lost in the library is the same one a good researcher imposes on themselves: a hard cap on trips before writing up what you have.
...
**Plan.** The agent decides its retrieval strategy: one search, a decomposed chain of searches, or — for queries it can answer without any lookup — no search at all. This is the router Adaptive RAG formalizes: classify query difficulty first, and only pay for the loop when needed.
...
The determinism cost is easy to underweight until it bites: a small rewording of the same question can send the agent down a different search path, back with an answer in a different shape. The mitigation isn't to relax the model's freedom but to constrain the *graph of possible paths* — frameworks like LangGraph or DSPy fix which states and transitions are legal, even though which one gets picked is still the model's call. Because the loop is the expensive path, the honest default is Adaptive RAG: classify the query first, route single-fact lookups straight to the linear pipeline, and reserve the loop — capped at 3–5 turns — for compound and multi-hop questions.
