...
Both paths in the diagram start from the same place: render each document page to an image (around 300 DPI). Follow the arrows left to right.
...
The narrative version: parsing pays per page, once, and lets everything downstream stay boring. Page-as-image pays continuously in storage and index machinery but wins on documents where layout is the meaning. The unified space is the simplest and the weakest on documents. Production systems today mostly run modality-aware parsing plus fusion as the workhorse, with page-as-image retrieval gaining ground fast for PDF-heavy corpora.
