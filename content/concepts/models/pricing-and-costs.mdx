...
**Top row — cache miss.** The request is a 2,000-token system prompt plus retrieved context, plus a 100-token user question: 2,100 input tokens at the standard $3.00/M rate. Because this is the first time the provider has seen this exact prefix, it also charges a one-time cache-write fee on the cacheable portion — typically 1.25x to 2x the standard input rate, depending on the TTL you request (a 5-minute cache is cheaper to write than a 1-hour one). The 400-token response bills at $15.00/M. Total: $0.0198 — more than the naive "$3.00 in, $15.00 out" math suggests, because that math forgets the write fee exists.
...
- **Reality with a 5-minute TTL and steady traffic:** roughly 85% of requests land inside the cache window (a hit at $0.0069) and 15% miss — TTL lapsed, or first call of a session (a miss at $0.0198, write fee included). Blended: 0.85 × $0.0069 + 0.15 × $0.0198 ≈ **$0.00884/request** → **$442/day**, about **$13,250/month** — roughly **28% below** the naive estimate, with no change to model, prompt, or traffic.
...
Same model, same rate card, three real costs — $18,450/month, $13,250/month, and $2,250-instead-of-$4,500 — none of it visible in the headline $3.00/$15.00 price.
