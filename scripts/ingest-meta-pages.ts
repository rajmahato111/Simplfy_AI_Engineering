...
const PAGES = [
  { file: "00-interview-prep/02-answer-frameworks.md", id: "frameworks", title: "Answer frameworks" },
...
function parseSections(md: string): MetaSection[] {
  ...
  for (const m of md.matchAll(/^## (.+)$/gm)) {
    ...
    const body = cleanBody(next < 0 ? rest : rest.slice(0, next)).slice(0, 6000);
    if (!body) continue;
...
async function main() {
  ...
  for (const spec of PAGES) {
    ...
    const summary = sections[0]?.body.slice(0, 400) ?? "";
    pages.push({
      id: spec.id,
      title: spec.title,
      summary,
